...
def parse_channel_with_sign():
    ...
    # 签名别名-密码
    key_alias_pw = option.key_alias_pw
    # 签名密码
    ...
    # 4：对齐的字节对齐数，这里设为4，表示按4字节对齐。
    # {option.apk_path}：输入的 APK 文件路径。
    # {sign_apk_path}：输出的已签名 APK 文件路径。
    cmd = f"zipalign -v -f 4 {option.apk_path} {sign_apk_path}"
    print(cmd)
    ret = os.system(cmd)
    if ret == 0:
        print("zipalign成功")
    ...
    # --ks-pass pass:{key_alias_pw}：指定密钥库密码，这里的 {key_alias_pw} 是指密钥库密码。
    ...
    cmd = f"apksigner sign --ks {key_file} --ks-key-alias {key_alias} --ks-pass pass:{key_alias_pw} --key-pass pass:{key_pw} {sign_apk_path}"
    ret = os.system(cmd)
    if ret == 0:
        print("sign成功")
    else:
    ...
    for root, dirs, files in os.walk(outputs_dir):
        for filename in files:
            ...
            # 测试替换效果可以打印 old_path 和 new_path 的值
